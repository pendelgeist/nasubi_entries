...
// Parse entries
const entriesRaw = fs.readFileSync('nasubi-entries.md', 'utf-8');
const entries = [];
for (const line of entriesRaw.split('\n')) {
  ...
  if (parts.length >= 4) {
    ...
    const prizeMonth = parts.length > 4 ? parts[4].trim() : '';
    let jpy = 0;
    ...
    entries.push({
      ...
      entryMonth: parts[3].trim(), prizeMonth, jpy,
...
// Parse winnings
const winRaw = fs.readFileSync('nasubi-winnings.md', 'utf-8');
const winnings = [];
for (const line of winRaw.split('\n')) {
  ...
  if (parts.length >= 3) {
    ...
    const month = parts.length > 3 ? parts[3].trim() : '';
...
// ROI for won items
const wonWithEffort = entries.filter(e => e.won && e.jpy > 0).map(e => ({
  name: e.en, postcards: e.entries, jpy: e.jpy, month: e.entryMonth,
  roi: e.entries > 0 ? e.jpy / e.entries : 0
}));
...
function categorize(en) {
  ...
  if (e.includes('beef') || e.includes('steak') || e.includes('pork') || e.includes('duck') || e.includes('crab') || e.includes('lobster') || e.includes('fish') || e.includes('salmon') || e.includes('eel') || e.includes('prawn') || e.includes('sea urchin') || e.includes('mackerel') || e.includes('ham') || e.includes('sausage') || e.includes('meat') || e.includes('matsutake') || e.includes('natto') || e.includes('watermelon') || e.includes('strawberr') || e.includes('melon') || e.includes('cherri') || e.includes('apple') || e.includes('grape') || e.includes('fruit') || e.includes('corn') || e.includes('tomato') || e.includes('cucumber') || e.includes('vegetable') || e.includes('mushroom') || e.includes('pickle') || e.includes('tofu') || e.includes('bean') || e.includes('miso') || e.includes('soy') || e.includes('seaweed') || e.includes('nori') || e.includes('horse meat') || e.includes('edamame') || e.includes('gyoza')) return 'Food (fresh/specialty)';
  ...
  if (e.includes('shampoo') || e.includes('soap') || e.includes('cosmetic') || e.includes('cream') || e.includes('perfume') || e.includes('cologne') || e.includes('nail') || e.includes('hair') || e.includes('toothbrush') || e.includes('toothpaste') || e.includes('shaver') || e.includes('dryer') || e.includes('beauty') || e.includes('lotion') || e.includes('oil blot')) return 'Personal Care/Beauty';
...
// Items entered most months (persistence)
const itemMonths = {};
for (const e of entries) {
  const key = e.en.toLowerCase().replace(/[^a-z]/g, '');
